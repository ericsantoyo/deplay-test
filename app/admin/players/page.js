...
const Players = () => {
  ...
  useEffect(() => {
    if (currentState === ADMIN_REQUEST_STATUS.IN_PROGRESS) {
      const percentage = Math.round(
        ((currentIndex - startingIndex) / (endingIndex - startingIndex)) * 100
      );
  ...
  return (
    <div className="pt-4 flex flex-col gap-4">
      <div>
        <h1 className="text-2xl font-bold text-center">Update Players</h1>
      </div>
      {/* <AdminNavigationBar active={ADMIN_NAVIGATION_ITEMS.PLAYERS} /> */}
      <Card className="flex flex-col p-6 w-1/3 justify-center items-center mx-auto bg-neutral-300 shadow">
        {currentState === ADMIN_REQUEST_STATUS.IDLE && (
          <div className="">
            <p className="text-xl p-2 text-center">Starting Index</p>
            <input
              className="p-1 rounded text-center outline-none border"
              placeholder="starting index"
              value={startingIndex}
              onChange={handleStartingIndexChange}
            />
            <p className="text-xl p-2 text-center">Ending Index</p>
            <input
              className="p-1 rounded text-center outline-none border"
              placeholder="ending index"
              value={endingIndex}
              onChange={handleEndingIndexChange}
            />
          </div>
        )}
